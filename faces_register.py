...
class Face_Register:
    def __init__(self):
        ...
        # Tkinter GUI
        self.win = tk.Tk()
        self.win.title("Face Register : Smart Attendance System AI Based System")
    ...
    def GUI_info(self):
        ...
        # Step 4: Save current face in frame
        tk.Label(self.frame_right_info,font=self.font_step_title,text="Step 4: Close window to finish").grid(row=11, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

        tk.Button(self.frame_right_info,text='Close the window to finish',command=self.GUI_close_window).grid(row=12, column=0, columnspan=3, sticky=tk.W)
    ...
    def check_existing_faces_cnt(self):
        if os.listdir("data/data_faces_from_camera/"):
            ...
            person_list = [p for p in os.listdir("data/data_faces_from_camera/") 
                          if p.startswith('student_')]
            person_num_list = []
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char:
            self.current_face_dir = self.path_photos_from_camera + "student_" + str(self.existing_faces_cnt) + "_" + self.input_name_char
        else:
            self.current_face_dir = self.path_photos_from_camera + "student_" + str(self.existing_faces_cnt)
